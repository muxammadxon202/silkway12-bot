...
ADMIN_ID=<твой Telegram ID — узнать за 5 сек у @userinfobot>
POSTGRES_PASSWORD=<сгенерируй: openssl rand -base64 24>
DATABASE_URL=postgresql+asyncpg://silkway:<тот же пароль>@db:5432/silkway
ALLOWED_ORIGINS=https://muxammadxon202.github.io,https://silkway12.uz,https://www.silkway12.uz
API_DOMAIN=api.silkway12.uz
```

`BOT_TOKEN` и `ADMIN_ID` обязательны — бот не запустится без них (падает на
старте с понятной ошибкой). `ADMIN_ID` больше не назначается автоматически:
раньше первый, кто писал боту `/start`, становился админом и мог перехватить
все заявки с сайта — это было исправлено.

> `.env` держи только здесь. В git не попадает (см. `.gitignore`), но всё
> равно `chmod 600 .env`.
...
## 5. Проверить, что заявки идут на твой аккаунт

Открой бота в Telegram, отправь `/start` — он ответит «Ты админ». Если ADMIN_ID
в `.env` указан неверно, бот вообще не запустится (проверь `docker compose logs app`).
